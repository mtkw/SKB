...
@NamedQueries({
    ...
    @NamedQuery(name = "Transaction.findByConditionsId", query = "SELECT t FROM Transaction t WHERE t.transactionPK.conditionsId = :conditionsId")})
...
@NamedNativeQueries({
    ...
    @NamedNativeQuery(name = "Transaction.NativfindByCustomerAndGroupBy", query = "SELECT t.id_transaction, i.name_insurance, i.basic_rate, COALESCE(SUM(ic.value),0) as value FROM Transaction t"
            + " join Insurance i on t.insurance_id = i.id_insurance"
            + " join Insurance_conditions ic on t.conditions_id = ic.id_condition "
            + "WHERE t.customer_id = ?1 "
            + "GROUP BY t.id_transaction, i.basic_rate, t.customer_id, i.name_insurance", resultClass = TransactionPosition.class)
...
public class Transaction implements Serializable {
    ...
    @JoinColumn(name = "conditions_id", referencedColumnName = "id_condition", nullable = false, insertable = false, updatable = false)
    @ManyToOne(optional = false, fetch = FetchType.EAGER)
    ...
    public Transaction(int idTransaction, int customerId, int insuranceId, int conditionsId) {
        this.transactionPK = new TransactionPK(idTransaction, customerId, insuranceId, conditionsId);
    }
